...
class FastGAN(GAN):
    def build(self, input_shape):
        ...
        o128 = keras.layers.Activation("tanh")((conv(input_shape[-1], kernel_size=3, strides=1)(feat_128)))
        if input_shape[0] >= 256:
            feat_256 = SEBlock(nfc[256], [feat_16, UpBlock(nfc[256], feat_128)])
        if input_shape[0] == 256:
            o = keras.layers.Activation("tanh")((conv(input_shape[-1], kernel_size=3, strides=1)(feat_256)))
        if input_shape[0] >= 512:
            feat_512 = SEBlock(nfc[512], [feat_32, UpBlockComp(nfc[512], feat_256)])
        if input_shape[0] == 512:
            o = keras.layers.Activation("tanh")((conv(input_shape[-1], kernel_size=3, strides=1)(feat_512)))
        if input_shape[0] == 1024:
            feat_1024 = UpBlock(nfc[1024], feat_512)
            o = keras.layers.Activation("tanh")((conv(input_shape[-1], kernel_size=3, strides=1)(feat_1024)))
        ...
        img_128 = keras.layers.Input(shape=(128, 128, input_shape[-1]))
        img_ = AugmentLayer()(img)
        img_128_ = AugmentLayer()(img_128)
        if input_shape[0] == 256:
            feat_2 = conv(nfc[256], 3, 1)(img_)
            feat_2 = keras.layers.LeakyReLU(0.2)(feat_2)
        if input_shape[0] == 512:
            feat_2 = conv(nfc[512], 4, 2)(img_)
            feat_2 = keras.layers.LeakyReLU(0.2)(feat_2)
        if input_shape[0] == 1024:
            feat_2 = conv(nfc[1024], 4, 2)(img_)
            feat_2 = keras.layers.LeakyReLU(0.2)(feat_2)
        ...
        feat_small = conv(nfc[256], 4, 2)(img_128_)
    ...
    def summary(self, **kwargs):
        ...
        self._gen.summary(**kwargs)
        self.real_dis.summary(**kwargs)
        self.fake_dis.summary(**kwargs)

    def train_step(self, images):
        style_noise = tf.random.normal((tf.shape(images)[0], self.latent_dim))
        with tf.GradientTape(persistent=True) as tape:
            gen_img, gen_img128 = self._gen(style_noise, training=True)
            fake = self.fake_dis([gen_img, gen_img128])
            real, rec_img_big, rec_img_small = self.real_dis([images, tf.image.resize(images, (128, 128))])
            gen_loss = self._gen_loss(real, fake)
            dis_loss = self._dis_loss(real, fake)
            dis_loss += perceptual(rec_img_big, tf.image.resize(images, (rec_img_big.shape[1], rec_img_big.shape[1])))
            dis_loss += perceptual(rec_img_small, tf.image.resize(images, (rec_img_small.shape[1], rec_img_small.shape[1])))
        ...
        self.g_optimizer.apply_gradients(zip(gen_gradients, self._gen.trainable_variables))
        self.d_optimizer.apply_gradients(zip(dis_gradients, self.real_dis.trainable_variables))
